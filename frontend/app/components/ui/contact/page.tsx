...
export default function Contact() {
  ...
  const {
    ...
    data: emailResponse,
    error: emailError,
  } = useMutation<typeof emailFormData, EmailResponse>(
    "post",
    "/contact-email",
  );
  ...
  return (
    <>
      ...
      >
        ...
        <form
          className="relative z-10 flex flex-col gap-6 w-full max-w-md"
        ...
        >
          <label className={`${styles.label} flex flex-col text-lg font-bold`}>
            ...
            <input
              ...
              onChange={(e) =>
                setEmailFormData((prev) => ({ ...prev, name: e.target.value }))
              }
            />
          </label>
          <label className={`${styles.label} flex flex-col text-lg font-bold`}>
